...
interface SalonEditModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onUpdated?: (updatedSalon: SalonInfo) => void;
  onDelete?: (salon: SalonInfo) => void;
}

const PRESET_COVERS = [
  'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=1000&auto=format&fit=crop&q=80',
  'https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?w=1000&auto=format&fit=crop&q=80',
  'https://images.unsplash.com/photo-1585747860715-2ba37e788b70?w=1000&auto=format&fit=crop&q=80',
  'https://images.unsplash.com/photo-1560869713-7d0a29430803?w=1000&auto=format&fit=crop&q=80'
];
...
export const SalonEditModal: React.FC<SalonEditModalProps> = ({
  salon,
  isOpen,
  onClose,
  onUpdated,
  ...
  const [selectedServicesMap, setSelectedServicesMap] = useState<Record<string, PrestationState>>({});
  ...
  useEffect(() => {
    if (salon && isOpen) {
      ...
      setFormData({
        ...
        phone: salon.phone || '',
        email: salon.email || '',
        currency: salon.currency || 'FCFA',
        universe: salon.universe || salon.univers || 'mixte',
        openingDays: salon.openingHours?.days || 'Mardi au Samedi',
        openingHours: salon.openingHours?.hours || '09:00 - 19:30',
        coverImage: salon.coverImage || PRESET_COVERS[0],
        logo: salon.logo || PRESET_LOGOS[0]
      });
      setErrorMsg('');
      setSuccessMsg('');
      ...
      // Load all global catalog services AND the specific services assigned to this salon
      setIsLoadingServices(true);
      Promise.all([
        api.getServices(),
        api.getSalonServices(salon.id).catch(() => [])
      ])
        .then(([allServices, salonServices]) => {
          setAvailableCatalogServices(allServices);

          const initialMap: Record<string, PrestationState> = {};
          salonServices.forEach((srv) => {
            const srvId = srv.id_service || srv.id;
            initialMap[srvId] = {
              selected: true,
              cout: srv.price || srv.cout || 5000,
              duree: srv.duration || srv.duree || 30
            };
          });
          setSelectedServicesMap(initialMap);

          if (allServices.length > 0) {
            setSelectedCatalogServiceId(allServices[0].id);
            const firstSalonSrv = initialMap[allServices[0].id];
            setNewServiceForm({
              nom: allServices[0].nom || allServices[0].name,
              univers: allServices[0].univers || allServices[0].universe || 'Dame',
              description: allServices[0].description || '',
              cout: firstSalonSrv?.cout || allServices[0].price || 5000,
              duree: firstSalonSrv?.duree || allServices[0].duration || 30
            });
          }
        })
        .catch((err) => {
          console.error('Erreur chargement prestations du salon:', err);
        })
        .finally(() => {
  ...
  const handleFileUpload = (file: File, field: 'coverImage' | 'logo') => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setErrorMsg('La taille ne doit pas dépasser 10 Mo.');
      return;
    }
  ...
  const handleSelectAllServices = (selectAll: boolean) => {
    ...
    setSelectedServicesMap(prev => {
      const nextMap: Record<string, PrestationState> = {};
  ...
  const handleSaveCustomService = async (e?: React.SyntheticEvent) => {
    ...
    try {
      ...
      setIsCreatingCustomService(false);
      setPrestationFeedback(`✓ Prestation "${created.nom || created.name}" ajoutée au catalogue et à ce salon !`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.address.trim() || !formData.city.trim()) {
      setErrorMsg('Le nom, la ville et l\'adresse sont obligatoires.');
      return;
    }
    ...
    try {
      const updated = await updateSalon(salon.id, {
        ...
        logo: formData.logo,
        openingHours: {
          days: formData.openingDays,
          hours: formData.openingHours
        },
        prestations
      });

      setSuccessMsg('Fiche du salon et prestations mises à jour avec succès !');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fadeIn">
      <div className="theme-bg-card border theme-border rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden animate-scaleIn my-auto relative flex flex-col max-h-[94vh]">
        
        {/* Header */}
        <div className="p-4 sm:p-5 border-b theme-border bg-gradient-to-r from-amber-500/10 via-transparent to-transparent flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold theme-text-primary font-serif">
                Mettre à jour la fiche du Salon
              </h2>
        ...
        <div className="flex border-b theme-border px-4 bg-amber-500/5 flex-shrink-0">
          ...
          >
            ...
            <span>Prestations du Salon</span>
        ...
        >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 rounded-xl text-xs theme-bg-subtle border theme-border theme-text-primary focus:outline-none focus:ring-2 focus:ring-amber-500 font-semibold"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="w-full pl-9 pr-3 py-2 rounded-xl text-xs theme-bg-subtle border theme-border theme-text-primary focus:outline-none focus:ring-2 focus:ring-amber-500 font-semibold cursor-pointer"
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="femme">Dame 👩 (Beauté, Soins, Onglerie)</option>
                    <option value="homme">Homme 👨 (Barber Club, Coupe Homme)</option>
                    <option value="enfant">Enfant & Ado 🧒 (Junior, Nattes)</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Soins haute couture & Coiffure prestige"
                    className="w-full px-3 py-2 rounded-xl text-xs theme-bg-subtle border theme-border theme-text-primary focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 rounded-xl text-xs theme-bg-subtle border theme-border theme-text-primary focus:outline-none focus:ring-2 focus:ring-amber-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      className="text-[11px] font-bold theme-text-accent flex items-center gap-1 hover:underline cursor-pointer"
                    >
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {PRESET_COVERS.map((cov, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => handleChange('coverImage', cov)}
                        className={`relative h-12 rounded-xl overflow-hidden border transition cursor-pointer ${
                          formData.coverImage === cov ? 'ring-2 ring-amber-500 scale-95' : 'opacity-60 hover:opacity-100'
                        }`}
                      >
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {PRESET_LOGOS.map((lg, idx) => (
                      ...
                      >
                        <img src={lg} alt="logo" className="w-full h-full object-cover" />
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {activeTab === 'services' && (
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="p-3.5 rounded-2xl theme-bg-card border theme-border shadow-sm space-y-3">
                <div className="flex items-center justify-between border-b theme-border pb-2">
                  <span className="text-xs font-bold theme-text-primary flex items-center gap-1.5">
                    ...
                    <span>Ajouter ou modifier une prestation pour ce salon</span>
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    ⚡ Tout importer ({availableCatalogServices.length})
                  </button>
                </div>
                ...
                <div className="space-y-2.5">
                  {/* Select service from services table */}
                  <div>
                    <label className="block text-[11px] font-bold theme-text-primary mb-1">
                      Prestation (depuis le catalogue des services) *
                    </label>
                    <select
                      ...
                      <optgroup label="✨ Prestations du Catalogue">
                        {availableCatalogServices.map(srv => (
                          <option key={srv.id} value={srv.id}>
                            {srv.nom || srv.name} — {srv.univers || srv.universe || 'Mixte'} {selectedServicesMap[srv.id]?.selected ? '✓ (Déjà dans ce salon)' : ''}
                          </option>
                        ))}
                      </optgroup>
                  ...
                  {/* Formulaire spécifique si prestation personnalisée */}
                    ...
                    >
                      ...
                      <span>
                        {selectedServicesMap[selectedCatalogServiceId || '']?.selected
                          ...
                          : 'Ajouter cette prestation à cet établissement'}
                      </span>
                    </button>
                  </div>
                </div>
              ...
              {/* TABLEAU DES PRESTATIONS DU SALON */}
                ...
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2">
                    <h4 className="text-xs font-bold theme-text-primary flex items-center gap-1.5">
                      ...
                      <span>Tableau des prestations proposées ({selectedCount})</span>
                    </h4>
                  </div>
                  ...
                  <div className="p-6 rounded-2xl border border-dashed theme-border text-center space-y-2 theme-bg-subtle/30">
                    ...
                    <div>
                      <p className="text-xs font-bold theme-text-primary">
                        Aucune prestation configurée pour ce salon
                      </p>
                      <p className="text-[11px] theme-text-secondary mt-0.5 max-w-xs mx-auto">
                        Sélectionnez une prestation ci-dessus, ajustez le tarif et la durée, puis cliquez sur <strong>"Ajouter cette prestation"</strong>.
                      </p>
                    </div>
                  </div>
                ) : (
          ...
          {/* Footer Buttons */}
          <div className="flex items-center justify-between pt-3 border-t theme-border flex-shrink-0">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
              ...
              {onDelete && salon && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">Supprimer l'établissement</span>
                </button>
              )}
            </div>
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-300 text-slate-950 font-bold text-xs shadow-lg shadow-amber-500/20 active:scale-95 transition-all flex items-center gap-2 cursor-pointer uppercase tracking-wider"
            >
              {isSubmitting ? (
              ...
              <span>Enregistrer les modifications ({selectedCount} prest.)</span>
            </button>
          </div>
        </form>
      </div>
